...
Algo::Algo(std::map<int, Algoinfo>* algoMap)
{
    this->add_algo(algoMap);
}
const uint8_t Algo::odd_parity_lut[256] = {
...
void Algo::des_set_odd_parity(uint8_t* key, size_t length)
{
    for (size_t i = 0; i < length; ++i)
    {
        key[i] = odd_parity_lut[key[i]];
    }
}
void Algo::add_algo(std::map<int, Algoinfo>* Algo)
{
    (*Algo)[0x6601] = { "DES", 64 / 8, 64 / 8, 64 / 8, 0, 0, &(this->des_set_odd_parity) };
    (*Algo)[0x6603] = { "DES3", 192 / 8, 64 / 8, 64 / 8, 0, 0, &(this->des_set_odd_parity) };
    (*Algo)[0x6611] = { "AES", 128 / 8, 128 / 8, 128 / 8, 0, 0, nullptr };
    (*Algo)[0x660e] = { "AES-128", 128 / 8, 128 / 8, 128 / 8, 0, 0, nullptr };
    (*Algo)[0x660f] = { "AES-192", 192 / 8, 128 / 8, 128 / 8, 0, 0, nullptr };
    (*Algo)[0x6610] = { "AES-256", 256 / 8, 128 / 8, 128 / 8, 0, 0, nullptr };

    (*Algo)[0x8009] = { "HMAC", 0, 512 / 8, 0, 0, 160 / 8, nullptr };
    (*Algo)[0x8003] = { "MD5", 0, 512 / 8, 0, 0, 128 / 8, nullptr };
    (*Algo)[0x8004] = { "SHA1", 0, 512 / 8, 0, 0, 160 / 8, nullptr };
    (*Algo)[0x800c] = { "SHA256", 0, 512 / 8, 0, 0, 256 / 8, nullptr };
    (*Algo)[0x800d] = { "SHA384", 0, 1024 / 8, 0, 0, 384 / 8, nullptr };
    (*Algo)[0x800e] = { "SHA512", 0, 1024 / 8, 0, 0, 512 / 8, nullptr };
}